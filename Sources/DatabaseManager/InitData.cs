...
namespace NRO_Server.DatabaseManager
{
    public class InitData
    {
                ...
                if (command != null)
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            short tNSM = (short)reader.GetInt32(9);
		...
		private static IEnumerable<sbyte> SetupItemTemplateNew()
        {
            List<sbyte> data;
            var writer = new MyWriter();
            if (Cache.Gi().ITEM_TEMPLATES.Count - Manager.gI().ItemOld == 0)
            {
                writer.WriteShort(0);
                writer.WriteShort(0);
            }
            else
            {
                ...
                for (var i = Manager.gI().ItemOld; i < Cache.Gi().ITEM_TEMPLATES.Count; i++)
                {
		...
		private static IEnumerable<sbyte> SetupVersionIcon()
        {
            ...
            var writer = new MyWriter();
            var iconMax = 15000;
            Console.WriteLine("LOADING ICON: [STATUS: LOADING]");
            ...
            data = writer.GetData().ToList();
            writer.Close();
            Console.WriteLine("LOADING ICON: [STATUS: SUCCESS]: " + iconMax);
            return data;
        }
    }
}