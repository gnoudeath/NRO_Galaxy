...
namespace NRO_Server.DatabaseManager.Player
{
    public class UserDB
    {
        ...
        public static void AddVND(Model.Player player, int amount)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    using (DbCommand command = DbContext.gI()?.Connection.CreateCommand())
                    {
                        command.CommandText = $"UPDATE `user` SET `vnd`=`vnd`+{amount} WHERE `id` = '{player.Id}' LIMIT 1;";
                        command.ExecuteNonQuery();
                    }
                }
                catch (Exception e)
                {
                    Server.Gi().Logger.Error($"Error AddVND User: {e.Message}\n{e.StackTrace}");
                }
                finally
                {
                    DbContext.gI()?.ConnectToAccount();
