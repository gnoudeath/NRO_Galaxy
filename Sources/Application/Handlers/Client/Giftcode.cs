    ...
    public static class Giftcode
    {
        ...
        private static void UseCode(Model.Character.Character character, string code, int codeType)
        {
            if (codeType == 0)//Tân thủ
            {
                ...
                thoivang.Quantity = 20; //thỏi vàng
                character.CharacterHandler.AddItemToBag(true, thoivang, "Giftcode");
                // 10 vé quay
                character.PlusDiamond(100000); // ngọc
                // 3678
                character.CharacterHandler.SendMessage(Service.SendBag(character));
                //character.CharacterHandler.SendMessage(Service.OpenUiSay((short)character.ShopId, @"Đã nhận thành công 200 tỏi vàng, 800k ngọc . Chúc bạn vui vẻ cùng NRO Galaxy"));
            } 
            ...
            //    character.CharacterHandler.SendMessage(Service.OpenUiSay((short)character.ShopId, @"Đã nhận thành công 2tr Ngọc"));
            //}
            else if (codeType == 1)
            {
                // 10 thoỉ vàng
                ...
                character.CharacterHandler.SendMessage(Service.SendBag(character));
                character.CharacterHandler.SendMessage(Service.OpenUiSay((short)character.ShopId, @"Đã nhận thành công 150 thỏi vàng,99 viên ngọc rồng 3 sao. Chúc bạn chơi game vui vẻ tại Ngọc Rồng Galaxy"));
            }
            else if (codeType == 5)
            {
                // 10 thoỉ vàng
                ...
                character.CharacterHandler.SendMessage(Service.SendBag(character));
                character.CharacterHandler.SendMessage(Service.OpenUiSay((short)character.ShopId, @"Đã nhận thành công 100 thỏi vàng. Quà đền bù"));
            }
            // else if (codeType == 2)//NRO Galaxy Bao tri code bảo trì reset tài khoản
            // {
            ...
            //  else if (codeType == 1)
            //  {
            //      var thoivang = ItemCache.GetItemDefault((short)711);
            //      thoivang.Quantity = 1;
            //      character.CharacterHandler.AddItemToBag(true, thoivang, "Giftcode");
            //      // 10 viên 3 sao
            //      var nr3s = ItemCache.GetItemDefault((short)16);
            ...
            //      // 3678
            //      character.CharacterHandler.SendMessage(Service.SendBag(character));
            //      character.CharacterHandler.SendMessage(Service.OpenUiSay((short)character.ShopId, @"Đã nhận thành công cải trang và 15v ngọc rồng 3 sao."));
            //  }
            // else if (codeType == 9)
            // {
            //     var thoivang = ItemCache.GetItemDefault((short)457);
            ...
            //     var hlt = ItemCache.GetItemDefault((short)1087);
            //     hlt.Quantity = 99;
