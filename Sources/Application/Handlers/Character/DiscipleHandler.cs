...
namespace NRO_Server.Application.Handlers.Character
{
    public class DiscipleHandler : ICharacterHandler
    {
        ...
        public void SetInfoSet()
        {
			...
			Disciple.InfoSet.IsFullSetHuyDiet = true;
            for (int i = 0; i < 5; i++)
            {
                if (Disciple.ItemBody[i] == null || Disciple.ItemBody[i].Id > 662 || Disciple.ItemBody[i].Id < 650)
                {
                    Disciple.InfoSet.IsFullSetHuyDiet = false;
                    break;
                }
            }
        ...
        public void PlusPoint(IMonster monster, int damage)
        {
            ...
            //ệ ma bư cộng chỉ số khi up cho sư phụ cao hơn
               if (Disciple.Type == 2)
               {
