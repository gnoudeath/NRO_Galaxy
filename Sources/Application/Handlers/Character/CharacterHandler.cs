...
namespace NRO_Server.Application.Handlers.Character
{
    public class CharacterHandler : ICharacterHandler
    {
        ...
        private void UpdateAntiChangeServerTime(string reason = "")
        {
            ...
            if ((Character.InfoChar.ThoiGianDoiMayChu - timeServer) < 100000)
            {
                Character.InfoChar.ThoiGianDoiMayChu = timeServer + 180000;
            }
        ...
        public void UpdateEffect(long timeServer)
        {
            ...
            if (Character.InfoBuff.CuongNo2Time < timeServer && Character.InfoBuff.CuongNo2)
            {
        ...
        {
			if (Character.InfoChar.pickDuaBe)
            {
        ...
        public void SetInfoSet()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (Character.ItemBody[i] == null || Character.ItemBody[i].Id > 662 || Character.ItemBody[i].Id < 650)
                {
        ...
        public void SetInfoBuff()
        {
            ...
            if (Character.InfoBuff.CuongNo2Time > timeServer)
            {
                var giayConLai = (Character.InfoBuff.CuongNo2Time - timeServer)/1000;
                if (giayConLai <= 0) giayConLai = 0;
        ...
        public void SetHpFull()
        {
            ...
            if (Character.InfoChar.Fusion.IsFusion && Character.Disciple != null) {
                ...
                // Đệ S Broly +130%
				else if (Character.Disciple.Type == 3)
                {
                    hp += (disHP + (disHP*30/100));
                }
                // Đệ S Broly +140%
        ...
        public void SetMpFull()
        {
            ...
            if (Character.InfoChar.Fusion.IsFusion && Character.Disciple != null) {
                ...
                {
                    mp += (Character.Disciple.MpFull + (Character.Disciple.MpFull*30/100));
                }
				else if (Character.Disciple.Type == 4)
                {
        ...
        public void PlusPoint(IMonster monster, int damage)
        {
            ...
            if(damage <= 0) return;
            var timeServer = ServerUtils.CurrentTimeMillis();
            long fixDmg = (long) ((damage) + (monster.OriginalHp * 0.04));
			...
			switch(Character.InfoChar.Task.Id)
			{
				case 7:
				{
					...
					{
						if (Character.InfoChar.Power > 16000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 16.000 sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
				case 8:
				{
					if(Character.InfoChar.Task.Index == 0)
					{
						if (Character.InfoChar.Power > 40000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 40.000 sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
				case 15:
				{
					if(Character.InfoChar.Task.Index == 0)
					{
						if (Character.InfoChar.Power > 200000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 200.000 sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
				case 16:
				{
					if(Character.InfoChar.Task.Index == 0)
					{
						if (Character.InfoChar.Power > 500000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 500.000 sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
				case 18:
				{
					if(Character.InfoChar.Task.Index == 0)
					{
						if (Character.InfoChar.Power > 1500000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 1.500.000 sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
				case 21:
				{
					if(Character.InfoChar.Task.Index == 0)
					{
						if (Character.InfoChar.Power > 50000000)
						{
                            Character.InfoChar.Task.Index++;
                            Character.CharacterHandler.SendMessage(Service.SendTask(Character));
                            Character.CharacterHandler.SendMessage(Service.ServerMessage("Đã đạt 50tr sức mạnh, hãy tiếp tục nhiệm vụ."));
						}
					}
					break;
				}
                    ...
                    switch (itemNew.Id)
                    {
						case 20:
						{
							if(Character.InfoChar.Task.Id == 8 && Character.InfoChar.Task.Index == 1)
							{
								Character.InfoChar.Task.Index++;
								SendMessage(Service.SendTask(Character));
								SendMessage(Service.ServerMessage("Đây có phải viên ngọc mà thầy nói không nhỉ???"));
							}
                            break;
						}
						case 85:
						{
							if(Character.InfoChar.Task.Id == 15 && Character.InfoChar.Task.Index == 1)
							{
								Character.InfoChar.Task.Index++;
								SendMessage(Service.SendTask(Character));
								SendMessage(Service.ServerMessage("Hừm, không biết ổng khoái truyện tranh hay ổng khoái đọc haiten nhỉ =))"));
							}
                            break;
						}
						case 73:
						{
							if(Character.InfoChar.Task.Id == 2 && Character.InfoChar.Task.Index == 0)
							{
								if(Character.InfoChar.Task.Count < 9)
								{
									Character.InfoChar.Task.Count++;
									Character.CharacterHandler.SendMessage(Service.SendTask(Character));
									Character.CharacterHandler.SendMessage(Service.ServerMessage("Bạn nhặt được "+Character.InfoChar.Task.Count+"/10 Đùi gà"));
								}
								else 
								{
									Character.InfoChar.Task.Index++;
						...
						case 78:
						{
                            Character.InfoChar.pickDuaBe = true;
                            UpdatePhukien();
                            Character.InfoChar.Task.Index = 2;
							Character.InfoChar.Task.Count = 0;
                            SendMessage(Service.SendTask(Character));
                            SendMessage(Service.ServerMessage("Wow một cậu bé dễ thương!!!"));
                            break;
						}
						case 380:
						{
							if(Character.InfoChar.Task.Id == 30 && Character.InfoChar.Task.Index == 1)
							{
								if(Character.InfoChar.Task.Count < 49)
								{
									Character.InfoChar.Task.Count++;
									Character.CharacterHandler.SendMessage(Service.SendTask(Character));
									Character.CharacterHandler.SendMessage(Service.ServerMessage("Bạn nhặt được "+Character.InfoChar.Task.Count+"/50 CSKB"));
								}
								else 
								{
									Character.InfoChar.Task.Index++;
        ...
        public object GetItemBagById(object itemId)
        {
            throw new NotImplementedException();
        }

        public void RemoveItemBag(object index, bool v1, string v2)
        {
            throw new NotImplementedException();
        }
    }
