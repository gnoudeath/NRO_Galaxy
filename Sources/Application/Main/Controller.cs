...
namespace NRO_Server.Application.Main
{
    public class Controller : IMessageHandler
    {
            ...
            try
            {
                ...
                switch (command)
                {
                        ...
                        if (character == null) return;
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    if (mapNext != null && DataCache.IdMapCold.Contains(mapNext.Id) && character.InfoChar.Power < 80000000000)
                                    {
									...
									if (character.InfoChar.Task.Id < 9 && mapNext.Id == 5)
									{
									...
									if (character.InfoChar.Task.Id < 25 && mapNext.Id > 91)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
                    ...
                    case -74:
                    {
                        ...
                        {
                            _session.SendMessage(Service.DialogMessage("Vui lòng tải đúng phiên bản mới trên Zalo để vào game"));
                            return;
                        }
                    ...
                    case -39:
                    {
                        ...
                        if (character.IsNextMap)
                        {
							...
							if (character.InfoChar.Task.Id == 3 && character.InfoChar.Task.Index == 1 && character.InfoChar.MapId == 42 + character.InfoChar.Gender)
                                {
                    ...
                    case 6:
                    {
                        ...
                        if (character != null)
                        {
                            ...
                            if (Server.Gi().StartServerTime > timeServer)
                            {
                                ...
                                character.CharacterHandler.SendMessage(Service.DialogMessage(string.Format(TextServer.gI().DELAY_RESTART_SEC, delay)));
                                return;
                            }

                            //Check if the user has made a purchase too quickly
                            var now = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                            var lastPurchaseTime = character.LastPurchaseTime;

                            if (lastPurchaseTime != null && now - lastPurchaseTime < 1)
                            {
                                character.CharacterHandler.SendMessage(Service.ServerMessage("Mua chậm thôi!!!"));
                                character.LastPurchaseTime = now + 2; // Set last purchase time to 2 seconds from now
                                return;
                            }
                            ...
                            Server.Gi().Logger.Debug($"Buy Item --------------- Type: {type} - Id Item: {id} - Quantity: {quantity}");
                            ...
                            character.LastPurchaseTime = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                        }
                                ...
                                if (mapTeleport != null)
                                {
                                    if (DataCache.IdMapCold.Contains(mapId))
                                    {
                                        if (character.InfoChar.Power < 80000000000) {
                                            character.CharacterHandler.SendMessage(Service.ServerMessage("Bạn cần đạt 80 tỷ sức mạnh mới có thể qua hành tinh Cold"));
                                            return;
                                        }
                                    }
									if (character.InfoChar.Task.Id < 9 && mapId == 5)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									if (character.InfoChar.Task.Id < 21 && mapId > 62)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									if (character.InfoChar.Task.Id < 22 && mapId > 72)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									if (character.InfoChar.Task.Id < 23 && mapId > 78)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									
									if (character.InfoChar.Task.Id < 25 && mapId > 91)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									if (character.InfoChar.Task.Id < 31 && mapId > 123)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
									if (character.InfoChar.Task.Id < 31 && mapId > 105)
									{
										character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn chưa thể đến khu vực này", false, character.InfoChar.Gender));
										return;
									}
                    ...
                    //         if (!_isWaiting)
                    //         {
                    //             Menu.Menu.OpenUiMenu(message.Reader.ReadShort(), (Character) character);
                    //             _isWaiting = true;
                    //             var timer = new Timer(20000);
                    //             timer.Elapsed += (sender, e) => _isWaiting = false;
                    //             timer.AutoReset = false;
                    //             timer.Start();
                    //         }
                    //         else
                    //         {
                    //             // do something to indicate that the user has to wait
                    //         }
                    //     }
                    //     break;
                    // }
                    ...
                    case 44:
                    {
                        ...
                        if (!text.Equals("") && character != null && role == 1)
                        {
                                ...
                                else if (text.Contains("kk"))
                                {
                                    var superBroly = new Boss();
                                    superBroly.CreateBoss(DataCache.BOSS_KUKU_TYPE, character.InfoChar.X, character.InfoChar.Y);
                                    superBroly.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(superBroly);
                                }
                                ...
                                else if (text.Contains("bad17"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_SUPER_ADR17_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bzms"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_ZAMASU_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bsoi3"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_BASIL_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bsoi2"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_LAVENDER_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bsoi1"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_BERGAMO_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bhit"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_HIT_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bclv"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_COLER_VANG_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bbr"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_BEERUS_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bkid"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_BUU_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bws"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_WHIS_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bp"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_TDT_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bso4"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_SO4_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bso3"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_SO3_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bso2"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_SO2_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bso1"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_SO1_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bt1"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_THOXANH_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bt2"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_THODO_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                else if (text.Contains("bt3"))
                                {
                                    var charReal = (Character)character;
                                    var boss = new Boss();
                                    boss.CreateBoss(DataCache.BOSS_THODEN_TYPE, charReal.InfoChar.X, charReal.InfoChar.Y);
                                    boss.CharacterHandler.SetUpInfo();
                                    character.Zone.ZoneHandler.AddBoss(boss);
                                }
                                #endregion
							...
							#region menuAdmin
                                ...
                                {
                                    var allPlayerInSever = "Tổng Player Trong Sever Đang Online Là: " + ClientManager.Gi().Characters.Count;
                                    character.CharacterHandler.SendMessage(Service.OpenUiConfirm(64, "Xin Chào Admin, " + allPlayerInSever + "\n|0|Sever Status: [ON]", MenuNpc.Gi().MenuAdmin[0], character.InfoChar.Gender));
                            ...
                            if (text.Contains("i"))
                            {
                                ...
                                try
                                {
                                    ...
                                    character.CharacterHandler.AddItemToBag(true, itemAdd, "buff");
                                    character.CharacterHandler.SendMessage(Service.SendBag(character));
                                    character.CharacterHandler.SendMessage(
                            ...
                            {
                                var itemTlt = ItemCache.GetItemDefault(1086);
                                var itemMap = new ItemMap(character.Id, itemTlt);
                                itemMap.X = character.InfoChar.X;
                                itemMap.Y = character.InfoChar.Y;
                                character.Zone.ZoneHandler.LeaveItemMap(itemMap);
                            ...
                            if (text.Contains("tn"))
                            {
                                if (character.InfoChar.Potential < 100000000000)
                                {
                                    character.CharacterHandler.PlusPotential(500000000);
                                    _session.SendMessage(Service.UpdateExp(1, 500000000));
                                }
                        ...
                        if (character != null)
                        {
                            ...
                            switch (type)
                            {
                                ...
                                case 2:
                                {
									...
									if (character.InfoChar.Task.Id == 30 && character.InfoChar.Task.Index == 0)
									{
										if(damageNew > 10000)
										{
											...
											character.CharacterHandler.SendMessage(Service.ServerMessage("Ái chà chà mạnh phết rồi đấy, ra quân thôiiii"));
										}
									}
                                    break;
                                }
                                ...
                                case 3:
                                {
                                    ...
                                    character.InfoChar.OriginalDefence += 1;
                                    break;
                                }
                                //Crit gốc
                                case 4:
							...
							if (character.InfoChar.Task.Id == 3 && character.InfoChar.Task.Index == 0)
                            {
                                character.InfoChar.Task.Index = 1;
        ...
        {
            // Tặng vàng, ngọc
            character.PlusGold(0);
            character.PlusDiamond(0);
        ...
        private void NextMap()
        {
            ...
            if (DataCache.IdMapCold.Contains(mapNext.Id) && character.InfoChar.Power < 80000000000)
            {
                character.CharacterHandler.SendMessage(Service.OpenUiSay(5, "Bạn cần đạt 80 tỷ sức mạnh mới có thể qua hành tinh Cold", false, character.InfoChar.Gender));
				...
				case 26:
				{
					if(character.InfoChar.Task.Index == 0)
					{
						if(mapNext.Id == 93)
						{
							...
							character.InfoChar.Task.Count = 0;
							character.CharacterHandler.SendMeMessage(Service.SendTask(character));
							character.CharacterHandler.SendMessage(Service.ServerMessage("Ối dồi ôi làng nước ơi, bọn chúng đến rồiiiiiii"));
						}
					}
					break;
				}
                            ...
                            case 5://nở trứng pet
                                {
                                    ...
                                    if (itemTrungLinhThu == null || itemTrungLinhThu.Id != 1086)
                                    {
                                        character.CharacterHandler.SendMessage(
                                    ...
                                    if (itemHonLinhThu == null || itemHonLinhThu.Id != 1085 || itemHonLinhThu.Quantity < 99)
                                    {
                                        character.CharacterHandler.SendMessage(
                                    ...
                                    if (thoiGianNoTrung != null)
                                    {
                                        ...
                                        if (itemThoiVang == null || itemThoiVang.Id != 457 || itemThoiVang.Quantity < 50)
                                        {
                                            character.CharacterHandler.SendMessage(
                                                Service.DialogMessage("Cần có 50 thỏi vàng để nở trứng sớm"));
                                            return Task.CompletedTask;
                                        }
                                    }
                                    ...
                                    if (arrIndexUi.Count == 3)
                                    {
                                        info += $"{ServerUtils.Color("blue")}Cần 50 thỏi vàng để nở trứng sớm";
                                    }
                                    info += $"{ServerUtils.Color("green")}Sẽ nở ra linh thú ngẫu nhiên hạng pet";
